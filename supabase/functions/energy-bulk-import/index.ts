...
async function importGekkoHistoricalData(params: GekkoHistoricalParams) {
  ...
  try {
    ...
    if (!historicalData?.list || !Array.isArray(historicalData.list)) {
      console.log('Available data keys:', Object.keys(historicalData));
      return new Response(
        JSON.stringify({ 
          error: 'No historical data found or invalid response format',
    ...
    const transformedData = historicalData.list.map((record: any, index: number) => {
    ...
    if (transformedData.length === 0) {
      return new Response(
        JSON.stringify({ 
          ...
          rawDataSample: historicalData.list?.slice(0, 3)
        }), 
        { 
          status: 400, 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
